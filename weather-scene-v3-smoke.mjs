...
try{
  ...
  for(const scene of ['cloud','rain','wind','wave']){
    ...
    result.scenes[scene]={
      ...
      motion:await stats('#motionCanvas'),
  ...
  const wind=(result.scenes.wind?.field?.visible||0)+(result.scenes.wind?.motion?.visible||0);
  const wave=result.scenes.wave?.field?.visible||0;
  result.ok=cloud>20&&wind>20&&wave>20&&result.flag&&result.pageErrors.length===0;
